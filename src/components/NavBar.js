...
import { GithubIcon, LinkedInIcon,SunIcon,TwitterIcon,MoonIcon } from "./Icons";
...
let CustomLink = ({ href, title, className = "" }) => {
...
let CustomMobileLink = ({ href, title, className = "",toggle }) => {
  const router = useRouter();
  const handleClick=()=>{
    toggle();
    router.push(href);
